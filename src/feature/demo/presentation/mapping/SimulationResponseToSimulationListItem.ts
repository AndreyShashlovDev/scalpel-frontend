...
import { ExchangeSimulationResultResponse } from '../../data/model/ExchangeSimulationResultResponse.ts'
...
export const SimulationResponseToSimulationListItem = (response: SimulationResponse) => {
  ...
  const latestExchange: ExchangeSimulationResultResponse | undefined = response.latestExchanges[0]
  const lastStableValue = latestExchange?.fromToken === response.currencyA.address
    ? latestExchange.exchangeAmount!
    : latestExchange?.resultExchangeAmount ?? response.initialAmountA
